...
public class ResetTokenCleanupService : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;
    private readonly ILogger<ResetTokenCleanupService> _logger;
    private readonly TimeSpan _cleanupInterval = TimeSpan.FromHours(24); // Run once per day

    public ResetTokenCleanupService(
        IServiceProvider serviceProvider,
        ILogger<ResetTokenCleanupService> logger)
    {
    ...
    private async Task CleanupExpiredTokensAsync()
    {
        ...
        var refreshTokenRepository = scope.ServiceProvider.GetRequiredService<IPasswordResetRepository>();
